...
MIG-024에서 event가 아닌 callback의 `$Fn.bind()`는 native `Function.prototype.bind()`로, 중앙 경계 밖 browser event는 `HuskyEvent.createHandler()`와 jQuery `.on()`/`.off()`로 전환했다. attach와 detach가 같은 handler reference를 사용하도록 정리했으며, 지원하지 않는 IE 전용 direct event 경로는 제거했다. MIG-025에서는 browser·OS capability를 [`BrowserCapabilities.js`](../../src/husky_framework/BrowserCapabilities.js)로 중앙화해 `$Agent` 호출을 제거했다. MIG-030에서는 [`DOM.js`](../../src/husky_framework/DOM.js)의 raw DOM lookup/HTML 생성 helper를 도입해 `$` 호출을 제거했다. MIG-031에서는 같은 helper의 selector 경계를 통해 `$$`/`cssquery`를 jQuery `.find()`/`.children()` 기반으로 전환하고 selector cache 옵션을 제거했다. MIG-032~034에서는 `$Element` wrapper를 iframe-local jQuery collection과 raw DOM API로 전환해 class/style/attribute, geometry와 DOM mutation을 처리했다. MIG-035/036에서는 Array/Hash wrapper를 native Array와 `Object.create(null)` 자료구조로 전환했다. MIG-037에서는 `Cookie.js`와 `DOMMetrics.js`를 추가해 cookie와 iframe document 측정을 대체하고, 문자열·날짜·XML 결과를 native API와 plain object로 전환했다. MIG-038에서는 HTML XHR과 JSONP를 iframe-local `window.jQuery.ajax()`로 전환하고 Jindo response wrapper를 제거했다. 활성 runtime source와 service의 `jindo.$`, `jindo.$$`, `jindo.cssquery`, `jindo.$Element`, `jindo.$Fn`, `jindo.$Agent`, `jindo.$A`, `jindo.$H`, `jindo.$Ajax`, `jindo.$S`, `jindo.$Document`, `jindo.$Cookie`, `jindo.$Date`, `jindo.$Json` 참조는 0개이고 migration guard가 재도입을 금지한다.
