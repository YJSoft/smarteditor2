...
function createSEditor2(elIRField, htParams, elSeAppContainer){
	var elAppContainer = (elSeAppContainer || $("#smart_editor2"));	
	var elEditingArea = elAppContainer.find("DIV.husky_seditor_editing_area_container").first();
	var oWYSIWYGIFrame = elEditingArea.find("IFRAME.se2_input_wysiwyg").first();
	var oIRTextarea = elIRField ? elIRField : elEditingArea.find("TEXTAREA.blind").first();
	var oHTMLSrc = elEditingArea.find("TEXTAREA.se2_input_htmlsrc").first();
	var oTextArea = elEditingArea.find("TEXTAREA.se2_input_text").first();
	...
	htParams.oNavigator = $.browser;				// navigator 객체 셋팅
	htParams.oNavigator.chrome = htParams.oNavigator.webkit;
	htParams.oNavigator.ie = htParams.oNavigator.msie;
	...
	if(htParams.bSkipXssFilter !== true){
		...
		oEditor.registerPlugin({
			...
			$ON_REGISTER_CONVERTERS : function() {
				var fXssFilter = $.proxy(function(sHtml){
